...
const NotificationsScreen = ({ navigation }) => {
  ...
  const handlePress = (notification) => {
    if (notification.comment) {
      const comment = portfolio.comments.find(
        (c) => c._id === notification.comment
      );
      if (!comment) {
        ToastAndroid.show("Comment has been deleted", ToastAndroid.SHORT);
        navigation.navigate("PortfolioShow", {
          itemId: notification.portfolio,
        });
            ...
            renderItem={({ item }) => {
              return (
                ...
                >
                  <ListItem
                    containerStyle={
                      item.read ? styles.readContainer : styles.unreadContainer
                    }
